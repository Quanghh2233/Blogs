...
func NewPostController(postService service.PostService, logger *logger.Logger) *postController {
...
// @Tags         posts
// @Produce      json
// @Success      200  {object}  serializer.PostResp
// @Router       /posts [get]
func (controller *postController) All(context *gin.Context) {
	...
	context.JSON(http.StatusOK, utils.GetResponse(gin.H{"totalPost": postCount, "posts": serializer.Response()}))
}
...
func (controller *postController) FindByID(context *gin.Context) {
	...
	if err != nil {
		context.JSON(http.StatusBadRequest, utils.ErrorsResponse("No param id was found"))
		controller.logger.Error().Err(err).Msg("No param id was found")
		return
	}
	post, err := controller.postService.FindById(postId)
	if err != nil {
		context.JSON(http.StatusNotFound, utils.ErrorsResponse("Post not found"))
...
// @Success      201  {object}  serializer.PostResp
// @Router       /posts [post]
...
func (controller *postController) DeleteById(context *gin.Context) {
	...
	if result.Error != nil {
		...
		context.JSON(http.StatusBadRequest, utils.ErrorsResponse("post does not exists"))

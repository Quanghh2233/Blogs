...
func (repo *commentRepo) AllCommentByPostId(limit string, offset string, postId uint) []model.Comment {
	...
	repo.db.Where("post_id = ?", postId).Order("id desc").Limit(commentLimit).Offset(commentOffset).Find(&comments)
	return comments
}
